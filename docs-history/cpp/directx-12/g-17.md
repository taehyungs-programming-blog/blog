...
* [Get Code 🌍](https://github.com/taehyung77/DirextX-Example/tree/14)
...
🙀 다른건 이해가 되는데 `GameObject`를 하나의 Object로 봐야하나? `GameObject`내에는 Textrue, Shader등이 포함될텐데?? -> 여러 리소스를 나름대로 커스터마이징한 하나의 Object라 생각하자(언리얼의 블루프린트와 동일한 개념!)
