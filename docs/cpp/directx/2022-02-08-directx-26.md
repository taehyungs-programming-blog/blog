...
float4 PS_Main(VS_OUT input) : SV_Target
{
    ...
    for (int i = 0; i < g_lightCount; ++i)
    {
        // 특히 빛 계산의 경우 함수내부를 보면알지만 연산량이 많은데 이걸 매번 해주는게 아쉽다
            // 가령 Spot light인데 범위가 해당객체까지 못온다면? Light계산이 필요없어지는데 Forward Shader에서는 구분이 불가능
...
🙀 이게 뭔... 사실 이걸 글로만 보고 이해하면 천재이다. 이렇게만 이해해도 100점이다. **지금까진 최종 결과를 Render Target에 넣었는데, 결과의 중간과정 까지 나눠서 여러 Render Target에 넣을 수 있게 하겠다.**
...
😺 위는 쉐이더이고 C++에서 쉐이더 연산의 결과를 받아야하니 코드 수정이 필요하다
...
private:
	...
	uint32							_rtCount;
	shared_ptr<Texture>				_dsTexture;     // Depth-Stencil은 별도 관리(Depth-Stencil도 Texture이기에 여기서 관리)
...
### 쉐이딩 중간과정이 저장된 Texture가 업데이트되는 과정
