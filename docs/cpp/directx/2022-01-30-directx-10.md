...
void Game::Init(const WindowInfo& info)
{
    ...
    // (문제.1) mesh, shader, texture가 몇개일지 모르는데 하나하나 Init(오래걸림)을 해준다고?
        // -> mesh, shader, texture를 재활용할 방안이 없나?
...
void Game::Update()
{
	...
	// ...
        ...
        // (문제.2) 결국 같은 오브젝트를 그리는데 shader, mesh의 Render, Update가 따로있다?
            // 오브젝트가 엄청나게 늘어난다면??
            // -> Render를 하나의 오브젝트에서 할 방안은?
