---
layout: default
title: "35. APC란? / Overlapped Callback 모델 구현"
parent: (IOCP)
grand_parent: C++
nav_order: 4
---
...
* APC(Asynchronous Procedure Call) : Thread마다 APC Queue가 존재하며 이 Queue를 APC Queue라 부른다, Thread가 APC Queue에 쌓인 함수를 처리할 준비가 됐을때(Alertable State) 해당 Thread는 APC Queue에 쌓인 모든 함수를 동작하게 된다.(자세한건 아래서 참조)
