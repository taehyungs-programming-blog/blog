    ...
    EProcessRemoteFunctionFlags& RemoteFunctionFlags)
{
    ...
    // 033 - Networking - ServerClassNetCache 

    // haker: we's like to find 'ClientSetHUD(UFunction)'
    // - see FClassNetCache::GetFromField(goto 034: ServerClassNetCache)
    const FFieldNetCache* FieldCache = ClassCache->GetFromField(Function);
    if (!FieldCache)
    {
        return;
    }
    // 035 - Networking - ServerClassNetCache - END

    // get the actor channel
    // haker: 'ActorChannel-Ch' is failed to find, so 'Ch == nullptr'
    // - see UNetConnection::FindActorChannelRef(goto 004: ServerProcessRemoteFunction)
    UActorChannel* Ch = Connection->FindActorChannelRef(Actor);
    if (!Ch)
    {
        // haker: note that 'bIsServer' is true in our case:
        // - see UNetDriver::IsLevelInitializedForActor(goto 005: ServerProcessRemoteFunction)
        // - depending on Actor's level visibility, we can create connection's ActorChannel to replicate connection's client
        //   *** note that we pass CreateChannelByName() with EChannelCreateFlags::OpenedLocally
        //       - for now, I am not sure what EChannelCreateFlags::OpenedLocally is exactly, but it looks like the indicator whether the server initiate ActorChannel or not
        // ***see UActorChannel(goto 016: ServerProcessRemoteFunction)
        if (bIsServer)
        {
        ...
        // haker: for client-side, creating ActorChannel is NOT allowed:
        // - initiative ActorChannel creation is done by server NOT client
        // - client also create ActorChannel but it is requested by the server:
        //   - if server orders to destroy ActorChannel, the client must follow!
        if (!Ch)
        {
            return;
        }
        ...
        if (bIsServer)
        {
            // haker: now assign 'Actor(LyraPlayerController)' to ActorChannel for each connection
            // - see UActorChannel::SetChannelActor(goto 018: ServerProcessRemoteFunction)
